...
commander
    .command('upgrade')
    .description('Install the last version')
    .action(function() {
        shell.exec('npm install -g ng1-decor-cli');
    });
    ...
    dns.lookupService('8.8.8.8', 53, function(err, hostname, service){
        if(err)
            ...
            try {
                ...
                depSpin.start();
                shell.exec('ncu -g ng1-decor-cli', () => {
                    depSpin.stop();
                });
            } catch (e) {}
        }
...
commander
    ...
    .option('-x, --reactive', 'Create a Reactive Component')
                ...
                switch (type) {
                    case 'component':
                        fs.mkdir(`${dirName || name}`, () => {
                            const rxjs = options.reactive;
                            addComponent(name, type, styleType, options, dirName, rxjs);
                        });
                        break;
                    case 'service':
...
function addComponent(name, type, styleType, options, dirName, rxjs) {
    // Typescript file
...
export class ${snakeToCamel(name, true)}Component ${rxjs ? 'extend RxjsComponent ' : ''}implements OnInit {
    ${content}
}
`);
    writeStreamTs.on('finish', () => {
